use std::string::FromUtf8Error;

/**
 * @Author: BearV
 * @Description: https://www.codewars.com/kata/546f922b54af40e1e90001da/train/rust
 * @Date: create in 2022/3/10 5:54 PM
 */

fn alphabet_position(text: &str) -> String {
    let mut str = String::new();
    let lower_text = text.to_ascii_lowercase();
    lower_text.into_bytes().iter().enumerate().for_each(|(i, x)| {
        let num = *x as i32;
        if num > 96 && num < 123 {
            let s = (num - 96).to_string();
            str.push_str(s.as_str());
            str.push_str(" ");
        };
    });
    println!("str is :{}", str);
    if str == "" {
        return str;
    }
    let (_, a) = str.as_str().as_bytes().split_last().unwrap();
    let som = String::from_utf8(Vec::from(a));
    match som {
        Ok(x) => x,
        Err(_) => "".to_string(),
    }
}


fn _alphabet_position(text: &str) -> String {
    text.to_lowercase()
        .chars()
        .filter(|c| c >= &'a' && c <= &'z')
        .map(|c| (c as u32 - 96).to_string())
        .collect::<Vec<String>>()
        .join(" ")
}


#[test]
fn returns_expected() {
    assert_eq!(
        alphabet_position("The sunset sets at twelve o' clock."),
        "20 8 5 19 21 14 19 5 20 19 5 20 19 1 20 20 23 5 12 22 5 15 3 12 15 3 11".to_string()
    );
    assert_eq!(
        alphabet_position("The narwhal bacons at midnight."),
        "20 8 5 14 1 18 23 8 1 12 2 1 3 15 14 19 1 20 13 9 4 14 9 7 8 20".to_string()
    );
}