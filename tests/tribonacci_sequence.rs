...
fn _tribonacci(signature: &[f64; 3], n: usize) -> Vec<f64> {
    match n {
        0 => [].to_vec(),
        n if n < 3 => signature[..n].to_vec(),
        _ => {
            let mut arr = signature.to_vec();
            for i in 0..n - 3 {
                new_num(&mut arr, i);
            }
            arr
...
fn new_num(arr: &mut Vec<f64>, n: usize) -> &mut Vec<f64> {
    let index = arr.len();
    if index == n {
        return arr;
    }
    arr.push(arr[index - 3] + arr[index - 2] + arr[index - 1]);
    arr
}
